...
//	}
    ...
    private static class Actions implements Action {
    ...
    protected void ensureApplicableLocation() {
        ...
        log.debug("tip's size {}, on {}", size, preferredLocation.x);
        ...
        log.debug("tip({},{}) , edge({},{})", maxX, maxY, right, bottom);
        if (maxY > bottom) {
            preferredLocation.y = bottom - size.height - 10;
        }
        log.debug("    => {}", preferredLocation);
