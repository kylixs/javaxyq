...
public abstract class PanelHandler implements PanelListener{
	...
	public void actionPerformed(ActionEvent evt) {
		try {
			...
			log.error("[PanelHandler]找不到事件的处理方法："+e.getMessage());
		} catch (Exception e) {
			log.error("[PanelHandler]执行事件时发生异常：{}", evt, e);
		}
	}
	
	synchronized public void dispose(PanelEvent evt) {
		log.info("dispose: "+this.getClass().getName());
		if(this.timer!=null) {
			this.timer.cancel();
			this.timer = null;
		}
	...
	public void initial(PanelEvent evt) {
		log.info("initial: "+this.getClass().getName());
	...
	public void help(ActionEvent evt) {
		log.info("help: "+this.getClass().getName());
	}
