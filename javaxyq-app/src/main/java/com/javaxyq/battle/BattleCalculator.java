...
public class BattleCalculator {
	...
	public Command attack(Command cmd) {
		...
		if(target == null) {
			log.info(cmd.getTarget().getName()+"已经退出战场，攻击无效。");
			return null;
		}
		if(src.hp <=0) {
			log.info(src.getName()+" 已倒下，无法发动攻击");
			return null;
		}else if(target.hp <= 0) {
			log.info(target.getName()+"已倒下，"+src.getName() +"的攻击无效");
			return null;
		}
		int hitpoints = 0;
		//判断是否命中
	...
	public Command magic(Command cmd) {
		...
		if(src.hp <=0) {
			...
			log.info(src.getName()+" 法力不足，施放法术失败！");
			return null;
		}
		src.mp -= usedmp;
		int hitpoints = 0;
	...
	public Command item(Command cmd) {
		...
		PlayerVO target = datas.get(cmd.getTarget());
		if(target == null) {
			log.info(cmd.getTarget().getName()+"已经退出战场，操作无效。");
			return null;
		}
		if(src.hp <=0) {
			log.info(src.getName()+" 已倒下，无法使用道具");
			return null;
		}else if(target.hp <= 0) {
			log.info(target.getName()+"已倒下，无法接受药物");
			return null;
		}		
